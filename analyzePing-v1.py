'''
This version of ping analysis computes median, min, and max RTT for each measurement file
The final results include two RTT values (v4 and v6) for each probe
'''
...
def get_rtt_stat(msm_file):
    ...
    if(med_rtt_list):
        return np.median(med_rtt_list), np.max(max_rtt_list), np.min(min_rtt_list)
    else:
...
def compute_stat_list(stat_value):
    ...
    rtt_diff = [x-y for (x,y) in zip(rtt_v4,rtt_v6)]
    return rtt_v4, rtt_v6, rtt_diff

def cdf_plot(rtt_diff):
    n_bins = 100000
    fig, ax = plt.subplots(figsize=(8, 6))
    # plot the cumulative histogram

    counts, bin_edges = np.histogram(rtt_diff, bins=n_bins)
...
cdf_plot(max_rtt_diff)
cdf_plot(med_rtt_diff)
cdf_plot(min_rtt_diff)
